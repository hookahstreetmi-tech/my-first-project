...
6. **Протокол Анализа (Check != Edit)**: Если Пользователь просит "проверить" или "посмотреть" — это команда на **ГЛУБОКИЙ АУДИТ**. Агент обязан:
   - Разобраться в логике и структуре.
   - Установить все связи и зависимости.
   - Найти корень ошибки или неэффективности.
   - Дать развернутый аналитический ответ.
   - **ЗАПРЕЩЕНО** вносить любые правки в процессе такого анализа. Реализация — только после отдельной команды.
...
Каждые **40 сообщений** Агент обязан создать выжимку в `AI_HISTORY/`:
